<?php
	...
	if(!$result = $db->query($sql)){
        ...
        $sql90 = "INSERT INTO log_errores (contexto,numero,error,ip) VALUES ('Amigos','-1','Error al obtener a todos los usuarios (visualizar solicitudes de amistad) excepto el propio de la BD.','$ip');";
    ...
    while($row = $result->fetch_assoc()){
        ...
        $sql3 = "SELECT id_usuario1, id_usuario2 FROM amigos WHERE (id_usuario1 = '$id_usuario' AND id_usuario2 = '$id_usuario_amigo' AND estado = 2) OR (id_usuario1 = '$id_usuario_amigo' AND id_usuario2 = '$id_usuario' AND estado = 2)";
                  
        if(!$result3 = $db->query($sql3)){

            $sql91 = "INSERT INTO log_errores (contexto,numero,error,ip) VALUES ('Amigos','-1','Error al obtener a los amigos del usuario (visualizar solicitudes de amistad) de la BD.','$ip');";
        ...
        while($row3 = $result3->fetch_assoc()){
            ...
            if($id_amigo1 != $id_usuario){
                $id_amigo = $id_amigo1;
                $estado = 2; //La peticion de amistad me la ha enviado otro usuario
            }

            else if($id_amigo2 != $id_usuario){
                $id_amigo = $id_amigo2;
                $estado = 1; //La peticion de amistad la he enviado yo
            }
            ...
            if(!$result4 = $db->query($sql4)){
                ...
                $sql92 = "INSERT INTO log_errores (contexto,numero,error,ip) VALUES ('Amigos','-1','Error al obtener al amigo del usuario (visualizar solicitudes de amistad) de la BD.','$ip');";

                if(!$result92 = $db->query($sql92)){

                }

                die($sql4.'2-Ha ocurrido un error durante la query [' . $db->error . '].');
            }
            ...
            while($row4 = $result4->fetch_assoc()){
                ...
                $sql20 = "SELECT * FROM notificaciones WHERE id_receptor = '$id_usuario' AND id_emisor = '$id_amigo' AND tipo = '6'";

                if(!$result20 = $db->query($sql20)){

                    $sql93 = "INSERT INTO log_errores (contexto,numero,error,ip) VALUES ('Amigos','-1','Error al obtener las notificaciones de solicitud de amistad del usuario (visualizar solicitudes de amistad) de la BD.','$ip');";
                ...
                while($row20 = $result20->fetch_assoc()){
                    $id_notificacion = $row4['id_notificacion'];
                }
                ...
                if(!$result2 = $db->query($sql2)){
                    ...
                    $sql94 = "INSERT INTO log_errores (contexto,numero,error,ip) VALUES ('Amigos','-1','Error al obtener la imagen del amigo del usuario (visualizar solicitudes de amistad) de la BD.','$ip');";
                ...
                while($row2 = $result2->fetch_assoc()){
                    ...
                    $amigos_usuario[] = array('id_amigo_usuario'=> $id_amigo_usuario, 'id_url_amigo'=> $id_url_amigo, 'usuario_amigo'=> $usuario_amigo, 'email_amigo'=> $email_amigo, 'foto_amigo'=> $foto_final_amigo, 'edad_amigo'=> $edad_amigo, 'sexo_amigo'=> $sexo_amigo, 'ultima_conexion_amigo'=> $ultima_conexion_amigo, 'estado'=> $estado, 'id_notificacion'=> $id_notificacion);                                            

<?php
    ...
    if(!$result = $db->query($sql)){
        ...
        $sql90 = "INSERT INTO log_errores (contexto,numero,error,ip) VALUES ('Amigos','-1','Error al obtener al amigo del usuario (rechazar solicitud de amistad) de la BD.','$ip');";
    ...
    while($row = $result->fetch_assoc()){
        ...
        $sql2 = "DELETE FROM amigos WHERE (id_usuario1='$id_usuario' AND id_usuario2='$id_amigo' AND estado=2) OR (id_usuario1='$id_amigo' AND id_usuario2='$id_usuario' AND estado=2)";

        //Si la consulta da cualquier error, se para aqui
        if(!$result2 = $db->query($sql2)) {

            $sql91 = "INSERT INTO log_errores (contexto,numero,error,ip) VALUES ('Amigos','-4','Error al eliminar al amigo del usuario (rechazar solicitud de amistad) de la BD.','$ip');";
